...
//				
				...
				// 첨부파일 형식으로 들어오게 되면 모든 파라미터는 바이트단위로 분해되서 전송
...
//				BoardVO bvo = new BoardVO(bno, title, content);
				...
				for(FileItem item : itemList) {
					...
					case "imageFile":
						...
						break;
					case "newFile":
						// 새로 추가된 파일 => 있을수도 있고, 없을 수도 있고.
						if(item.getSize()>0) {
							if(old_file != null) {
								// 기존파일이 있다면...
								// 파일 삭제작업 : 별도 핸들러로 작업진행
								FileRemoveHandler fr = new FileRemoveHandler();
								isOk = fr.deleteFile(savePath, old_file);
							}
							//새로운 파일 등록작업
							String fileName = item.getName();
							fileName = System.currentTimeMillis()+"_"+fileName;
